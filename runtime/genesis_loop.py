...
MSVB_KEYS = [
    ...
    "spinor","chirality","kappa","torsion","omega","L","extras"
]

def empty_msvb() -> Dict[str, Any]:
    return {
        "v_drift": v3(), "v_coherence": v3(), "v_bias": v3(), "v_friction": v3(),
        "v_gravity": v3(), "v_focus": v3(0,0,1), "spinor": v3(0,0,1),
        "chirality": 0, "kappa": 0.0, "torsion": 0.0, "omega": v3(), "L": v3(),
        "extras": {}
    }
...
def fieldstate_init() -> Dict[str, Any]:
    ...
    fs = {
        "time": {"dt_sys": 0.05, "breath": {"phase": 0.0, "state": "INHALE", "alpha": 0.0, "beta_mod": 1.0}},
        "global": {"mode6": "YELLOW", "cones6": [], "gate_open": 0.5},
        "layers": phi,
        "gravity_bus": {
            "v_drift": v3(), "v_coherence": v3(), "v_bias": v3(), "v_friction": v3(),
            "v_gravity": v3(), "v_focus": v3(0,1,0), "kappa": 0.0, "tau": 0.0,
            "harmonics": {"prime_entropy":1.0,"root_gain":1.0,"reciprocal_gain":1.0,"radial_gain":1.0},
            "weights_used": {}
        },
...
def ledger_commit(fs: Dict[str,Any], locks: List[Dict[str,Any]], rf_results: List[Dict[str,Any]]) -> None:
...
def step(fs: Dict[str,Any]) -> None:
    """One system tick."""
    # 1) Layers Φ0..Φ6
    update_phi0(fs); update_phi1(fs); update_phi2(fs)
    update_phi3(fs); update_phi4(fs); update_phi5(fs); update_phi6(fs)

    # 2) GravityBus
    gravity_bus_compose(fs)

    # 3) Layers Φ7..Φ9
    update_phi7(fs); update_phi8(fs); update_phi9(fs)
