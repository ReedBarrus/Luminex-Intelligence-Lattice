...
Vec3 = np.ndarray
def v(x=0.0, y=0.0, z=0.0) -> Vec3: return np.array([float(x), float(y), float(z)], float)
def v_zero() -> Vec3: return np.zeros(3, float)
def v_unit_x() -> Vec3: return np.array([1.0,0.0,0.0], float)
def v_unit_y() -> Vec3: return np.array([0.0,1.0,0.0], float)
def v_unit_z() -> Vec3: return np.array([0.0,0.0,1.0], float)

def vec3_to_list(u: Vec3) -> List[float]: return [float(u[0]), float(u[1]), float(u[2])]
def list_to_vec3(xs: Iterable[float]) -> Vec3:
    a = list(xs); 
    if len(a) != 3: return v_zero()
