...
class PhaseScriptKernel:
    ...
    # Handler registry
    _sim_handlers: dict[OpKind, Callable[[OpSpec], dict[str, Any]]] = field(
        default_factory=dict, init=False, repr=False
    )
    _commit_handlers: dict[OpKind, Callable[[OpSpec], dict[str, Any]]] = field(
        default_factory=dict, init=False, repr=False
    )
    ...
    def register_handlers(
        self,
        kind: OpKind,
        simulate_fn: Callable[[OpSpec], dict[str, Any]] | None = None,
        commit_fn: Callable[[OpSpec], dict[str, Any]] | None = None,
    ) -> None:
    ...
    def update(
        ...
        *,
        ...
        phi0_msvb: MSVB | None = None,  # Breath (gate_open)
        ...
        ops_budget = (
            self.base_ops_per_tick * clamp01(kappa6) * clamp01(gate_open) * clamp01(open_allowed)
        )
        ...
        for op in q:
            if budget_left < op.cost_est - 1e-9:
                results.append(
                    OpResult(status=OpStatus.SKIPPED, op=op, score=0.0, reason="no_budget")
                )
                continue

            plan = self._plan_op(
                op, gb_msvb=gb_msvb, cones=cones, mode=mode, phi9_decision=phi9_decision
            )

            if not plan.ethics_ok:
                results.append(
                    OpResult(
                        status=OpStatus.DENIED, op=op, score=plan.score, reason="ethics_denied"
                    )
                )
                continue
            ...
            if op.risk_score > self.max_risk:
                results.append(
                    OpResult(
                        status=OpStatus.DENIED, op=op, score=plan.score, reason="risk_over_max"
                    )
                )
                continue
            ...
            if plan.cone_factor <= 0.1:
                results.append(
                    OpResult(
                        status=OpStatus.SKIPPED, op=op, score=plan.score, reason="out_of_cones"
                    )
                )
                continue
            ...
            if mode_gate < 0.4:
                results.append(
                    OpResult(
                        status=OpStatus.SKIPPED, op=op, score=plan.score, reason="mode_gate_low"
                    )
                )
                continue
    ...
    def _plan_op(
        ...
        *,
        ...
        score = clamp01(0.50 * align + 0.30 * cone_factor + 0.20 * open_allowed) * (
            1.0 - 0.4 * op.risk_score
        )
...
if __name__ == "__main__":
    ...
    mirror_decision = {
        "open_level_allowed": 0.85,
        "ethical_floor": 0.2,
        "ethical_ceiling": 0.95,
        "verdict": "ALLOW",
    }
    ...
    ps.register_handlers(
        OpKind.NAME,
        simulate_fn=lambda op: {"preview": "ok"},
        commit_fn=lambda op: {"committed": True},
    )

    ps.enqueue(
        OpSpec(
            OpKind.NAME,
            intent_vec=unit(v(0.05, 0.0, 1.0)),
            payload={"label": "river"},
            priority=1.2,
            cost_est=1.0,
            risk_score=0.1,
    ...
    msvb, telem, results = ps.update(
        dt=0.02,
        gb_msvb=gb,
        phi6_msvb=phi6,
        phi9_decision=mirror_decision,
        phi0_msvb=phi0,
        cones=cones,
        ledger=LedgerClient(),
        mode="GREEN",
    ...
    print(
        "ops_budget=",
        round(telem["ops_budget"], 3),
        "committed=",
        [r.op.kind.value for r in results if r.status.value == "COMMITTED"],
    )
