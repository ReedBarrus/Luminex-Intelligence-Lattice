    ...
    def encode(self, msvbs: dict[str, MSVB]) -> np.ndarray:
        ...
        for name in self.layers:
            ...
            block = np.concatenate(
                [*(v if isinstance(v, np.ndarray) else np.array(v) for v in vecs), scal, omega]
            )
            feats.append(block)
...
class SGRU:
    ...
    def _mods(self, msvbs: dict[str, MSVB]) -> dict[str, float]:
        ...
        if phi3 is not None and gb is not None:
            v1 = unit(phi3.v_coherence)
            v2 = unit(gb.v_coherence)
    ...
    # ---- Utilities --------------------------------------------------
    def reset(self, h0: np.ndarray | None = None) -> None:
...
if __name__ == "__main__":
    ...
    msvbs = {
        "phi0": MSVB(
            v_drift=np.zeros(3),
            v_coherence=np.zeros(3),
            v_bias=np.zeros(3),
            v_friction=np.zeros(3),
            v_gravity=np.zeros(3),
            v_focus=np.zeros(3),
            L=np.zeros(3),
            spinor=np.array([0, 0, 1.0]),
            chirality=+1,
            extras={"gate_open": 0.9},
        ),
        "phi3": m([0.0, 0.0, 1.0]),
        "phi5": MSVB(
            v_drift=np.zeros(3),
        ...
        "phi7": MSVB(
            v_drift=np.zeros(3),
            v_coherence=np.array([0.0, 0.0, 1.0]),
            v_bias=np.zeros(3),
            v_friction=np.zeros(3),
            v_gravity=np.array([0.0, 0.0, 0.6]),
            v_focus=np.array([0.0, 0.0, 0.9]),
            L=np.zeros(3),
            spinor=np.array([0, 0, 1.0]),
            chirality=+1,
            extras={"open_level": 0.8},
        ),
        "gb": MSVB(
            v_drift=np.zeros(3),
            v_coherence=np.array([0.0, 0.0, 1.0]),
            v_bias=np.zeros(3),
            v_friction=np.zeros(3),
            v_gravity=np.array([0.2, 0.0, 0.6]),
            v_focus=np.array([0.0, 0.0, 1.0]),
            L=np.zeros(3),
            spinor=np.array([0, 0, 1.0]),
            chirality=+1,
        ),
    ...
    for i in range(5):
        ...
        print(
            ...
            i,
            "| |h|=",
            round(float(np.linalg.norm(h)), 3),
            "pred |grav|=",
            round(float(np.linalg.norm(preds["v_gravity"])), 3),
        )
