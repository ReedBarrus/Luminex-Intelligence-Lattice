...
def is_vec3(v: Any) -> bool:
    return (
        isinstance(v, (list, tuple)) and len(v) == 3 and all(isinstance(x, (int, float)) for x in v)
    )
...
def canonical_json(obj: Any) -> bytes:
    ...
    return json.dumps(
        obj, ensure_ascii=False, sort_keys=True, separators=(",", ":"), cls=CanonEncoder
    ).encode("utf-8")
...
class SymbolicLedger:
    ...
    def append(
        ...
        payload: dict[str, Any],
        *,
        consent_hash_value: str | None = None,
    ...
    def record_op_commit(
        ...
        *,
        ...
        return self.append(
            EntryType.OP, payload=op.to_json(), consent_hash_value=consent_hash_value, tags=tags
        )

    def record_lock(self, ev: LockEvent, *, tags: list[str] | None = None) -> LedgerEntry:
        return self.append(EntryType.LOCK, payload=ev.to_json(), tags=tags)

    def record_consent(
        self, rec: ConsentRecord, *, tags: list[str] | None = None
    ) -> LedgerEntry:
        return self.append(
            EntryType.CONSENT, payload=rec.to_json(), consent_hash_value=rec.consent_hash, tags=tags
        )

    def record_rftrace(self, rf: RFTrace, *, tags: list[str] | None = None) -> LedgerEntry:
        return self.append(EntryType.RFTRACE, payload=rf.to_json(), tags=tags)

    def record_note(self, text: str, *, tags: list[str] | None = None) -> LedgerEntry:
        return self.append(EntryType.NOTE, payload={"text": text}, tags=tags)

    # Retroactive "no" / rollback -----------------------------------
    def retroactive_no(
        self, target_entry_id: str, *, reason: str = "", plan: dict[str, Any] | None = None
    ) -> LedgerEntry:
        payload = {
            "target": target_entry_id,
            "reason": reason,
            "plan": plan or {"actions": ["PRUNE", "REPAIR"]},
        }
        return self.append(
            EntryType.ROLLBACK, payload=payload, tags=["retro_no"]
        )  # non‑destructive
...
if __name__ == "__main__":
    ...
    lock = LockEvent(
        ...
        lock_type="SymbolLock",
        acquired=True,
        metrics={"kappa": 0.82, "align": 0.9, "dwell": 0.7},
        vectors={"v_identity": vec3(0.0, 0.0, 1.0)},
    )
