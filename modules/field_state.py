...
from modules.types import MSVB
...
class FieldState:
    ...
    # --- time/breath ---
    @property
    def dt_sys(self) -> float:
        return float(self._fs["time"]["dt_sys"])

    @property
    def dt_phase(self) -> float:
        return float(self._fs["time"]["breath"].get("dt_phase", self.dt_sys))

    @property
    def alpha_breath(self) -> float:
        return float(self._fs["time"]["breath"]["alpha"])

    # --- global/coherence ---
    @property
    def mode6(self) -> str:
        return str(self._fs["global"].get("mode6", "YELLOW"))

    @property
    def cones6(self):
        return self._fs["global"].get("cones6", [])
